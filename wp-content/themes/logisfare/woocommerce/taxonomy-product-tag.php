...
$shop_tag_is_banner     = tw_get_option($customizer_ID, 'shop_tag_is_banner', 2);
$shop_tag_banner_block  = tw_get_option($customizer_ID, 'shop_tag_banner_block', -1);

$shop_sidebar           = tw_get_option($customizer_ID, 'shop_tag_sidebar', 3);
$shop_is_count          = tw_get_option($customizer_ID, 'shop_tag_is_count', 2);
$shop_is_sort           = tw_get_option($customizer_ID, 'shop_tag_is_sort', 2);
$shop_is_view_toggler   = tw_get_option($customizer_ID, 'shop_tag_is_view_toggler', 2);
$shop_default_view      = tw_get_option($customizer_ID, 'shop_tag_default_view', 1);
$shop_tag_is_pagination = tw_get_option($customizer_ID, 'shop_tag_is_pagination', 2);
$shop_tag_pagi_align    = tw_get_option($customizer_ID, 'shop_tag_pagi_align', 1);
$shop_tag_top_bloks     = tw_get_option($customizer_ID, 'shop_tag_top_bloks', []);
$shop_tag_bottom_bloks  = tw_get_option($customizer_ID, 'shop_tag_bottom_bloks', []);


if(class_exists('CSF')){
    $Meta_ID = '_logisfare_porduct_tag';

    $shop_tags_is_settings = tw_get_term_meta($termID, $Meta_ID, 'shop_tags_is_settings', 2);
    if($shop_tags_is_settings == 1){

        $shop_tags_is_banner    = tw_get_term_meta($termID, $Meta_ID, 'shop_tags_is_banner', 2);
        $shop_tags_banner_block = tw_get_term_meta($termID, $Meta_ID, 'shop_tags_banner_block', -1);

        $shop_tag_is_banner     = ($shop_tags_is_settings == 1 && $shop_tags_is_banner == 1 ? $shop_tags_is_banner : $shop_tag_is_banner);
        $shop_tag_banner_block  = ($shop_tags_is_settings == 1 && $shop_tags_banner_block > 0 ? $shop_tags_banner_block : $shop_tag_banner_block);
    }

}

if($shop_tag_is_banner == 1):
    if($shop_tag_banner_block > 0):
        if(class_exists('Themewar_Builder')):
            Themewar_Builder::render_template($shop_tag_banner_block);
        endif;
    else:
...
if (!empty($shop_tag_top_bloks)) {
    foreach ($shop_tag_top_bloks as $sb):
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="<?php echo esc_attr($colmnClass); ?>">
                ...
                <?php endif; ?>
                <?php if($shop_tag_is_pagination == 1) : ?>
                <div class="row shopPaginationRow">
                    <div class="col-lg-12">
                        <div class="logisfareShopTagPagination text-<?php echo esc_attr($shop_tag_pagi_align); ?>">
...
if (!empty($shop_tag_bottom_bloks)) {
    foreach ($shop_tag_bottom_bloks as $sb):
