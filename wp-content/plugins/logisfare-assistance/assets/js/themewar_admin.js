jQuery(document).ready(function($) {
    ...
    $(document).on('change', '.scat', function(e) {
        e.preventDefault();
        var cats = [];
        $parent = $(this).parent('.singleCat').parent('.categoryListContainer');
        $('.scat', $parent).each(function(){
            if($(this).is(':checked')){
                cats.push($(this).val());
            }
        });
        $('.catids', $parent).val(cats.join(','));
    });

    $('.addMoreProfile').on('click', function(e){
        e.preventDefault();
        var $btn = $(this);
        var $container = $('.socialProfilesContainer');
        var totalCount = $('.socialProfilesContainer .socialProfileRow').length;
        totalCount += 1;

        if(totalCount <= 10){
            var $html = $('.socialProfilesContainer .socialProfileRow:first-of-type').clone();
            $html.find('input, select').val('');
            $container.append($html);
            $('.socialProfilesContainer .socialProfileRow:last-of-type').append('<a href="javascript:void(0);" class="button button-danger removeProfile">Remove</a>');
        }
        
    });

    $('.socialProfilesContainer').on('click', '.removeProfile', function(e){
        e.preventDefault();
        $(this).parent('.socialProfileRow').remove();
    })
});
